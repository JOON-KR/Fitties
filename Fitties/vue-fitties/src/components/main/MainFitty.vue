<template>
  <div>
    ...
    <div v-if="isFitty" class="card-container">
      <div v-for="fitty in fittyList" :key="fitty.userId" class="card">
...
const isFitty = computed(() => store.fittyList && store.fittyList.length > 0);
// 데이터가 변경되면 자동으로 업데이트되도록 computed 사용
const fittyList = computed(() => {
  // 랜덤으로 3개의 아이템 선택
  const randomItems = [];
  const shuffledList = store.fittyList.sort(() => Math.random() - 0.5);
  for (let i = 0; i < 3; i++) {

<template>
  <div class="login-container">
    <form class="form" @submit.prevent="submitForm">
      <h3 class="title-animated">INBODY INFO</h3>

      <button type="button" @click="toggleForm" class="button-toggle">
        {{ isMainForm ? "빈 폼으로 전환" : "주요 폼으로 전환" }}
      </button>
      ...
      <transition name="fade">
        <div v-if="isMainForm" key="mainForm">
          <div class="flex-column">
            <label for="image" class="inbo"><strong>인바디 정보</strong></label>
            <div class="inbody-profile">
              <div class="card" @click="handleSvgClick($event)">
                <input
                  id="uploadFileTest"
                  type="file"
                  ref="fileInput"
        ...
        <div v-else key="customForm">
          ...
          <div class="flex-column">
            <label for="muscle"><strong>근육량</strong></label>
            <div class="inputForm">
              <input
                type="number"
                step="0.01"
                id="muscle"
                v-model.number="form.muscle"
                class="input"
          ...
          <div class="flex-column">
            <label for="fat"><strong>체지방량</strong></label>
            <div class="inputForm">
              <input
                type="number"
                step="0.01"
          ...
          <div class="flex-column">
            <label for="bmi"><strong>BMI</strong></label>
            <div class="inputForm">
              <input
                type="number"
                step="0.01"
          ...
          <div class="flex-column">
            <label for="fatPer"><strong>체지방률</strong></label>
            <div class="inputForm">
              <input
                type="number"
                step="0.01"
                id="fatPer"
                v-model.number="form.fatPer"
                class="input"
          ...
          <div class="flex-column">
            <label for="weight"><strong>체중</strong></label>
            <div class="inputForm">
              <input
                type="number"
                step="0.01"
      ...
      <button type="submit" class="button-submit">프로필 등록</button>
    </form>
  </div>
</template>
...
const fileTmp = ref(null);
const customField = ref(""); // 커스텀 필드 상태 변수 추가
const store = useUserStore();
const isMainForm = ref(true); // 현재 폼 상태를 나타내는 변수
const selectedFile = ref(null);
const toggleForm = () => {
  isMainForm.value = !isMainForm.value;
};

const submitForm = async () => {
  try {
    const formData = new FormData();
    formData.append("file", fileTmp.value);
    ...
    await store.addUserProfile(newProfile);
...
const handleFileChange = async (event) => {
  ...
  test.value = selectedFile.value.name;
  ...
  const responseData = response2.data; // 서버 응답 데이터
  form.value.muscle = responseData[0];
  form.value.fat = responseData[1];
  form.value.bmi = responseData[2];
  form.value.fatPer = parseFloat(responseData[3].slice(0, -1));
  form.value.weight = responseData[4];
};

const handleFileimage = async (event) => {
  // 선택된 파일 객체
  fileTmp.value = event.target.files[0]; //; form 객체에 파일 객체 저장
};
const handleSvgClick = (event) => {
  // console.log(event.target);
  const inbody = document.querySelector("#uploadFileTest");
  console.log(inbody);
  inbody.click();

  // const fileInput = document.querySelector("#uploadFileTest");
...
//   // input 요소 표시
//   const fileInput = card.querySelector("#uploadFileTest");
...
.create-profile-form form input[type="file"],
.create-profile-form form input {
  padding: 10px;
  width: 100%;
  border-radius: 5px;
...
.card {
  ...
  position: relative;
  transition: 0.2s;
  margin: auto; /* 부모의 영역에서 가운데로 오게 함*/
}
...
.card:hover {
  ...
  transition: 0.2s;
  filter: drop-shadow(0px 0px 10px rgb(255, 253, 208));
}
...
.card::before {
  ...
  margin-left: 10%;
  filter: drop-shadow(0px 0px 10px rgb(9, 26, 82))
    drop-shadow(0px 0px 10px rgb(189, 185, 185));
}

.card svg {
  filter: drop-shadow(0px 0px 10px #bbbbbb) drop-shadow(0px 0px 50px #bbbbbb);
}
